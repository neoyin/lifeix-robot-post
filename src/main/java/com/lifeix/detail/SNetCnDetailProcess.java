...
import com.lifeix.spider.SNetCnSpider;
...
public class SNetCnDetailProcess {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(SNetCnDetailProcess.class);
	...
	public static void dowithData(){
		...
		String fileName = dir+"/temp_"+SNetCnSpider.www_url;
	...
	public static void getDoveboxDataFromUrl(String url){
		...
		for (int j = 1; j < 10; j++) {
			...
			if (j>1) {
				tempURL = pageUrl.replace(".html", "_"+j+".html");
			}
			...
			for (Element element : elements) {
				...
				if (name.endsWith("description")) {
					desc = content.replace(SNetCnSpider.www_url, "");
				}
			}
			
			Elements images = document.select(".water");
			for (int i = 0; i < images.size(); i++) {
				Element  element =  images.get(i);
				String src ="http://"+SNetCnSpider.www_url+ element.attr("src");
				image +=src+",";
			}	
			if (j==1) {
				title=document.title();
				title = title.substring(0,title.indexOf("_七色空间")) ;
			}
	...
	private static void saveDoveData(String url,String str){
		...
		String fileName = path +SNetCnSpider.www_url+"/post_"+suffix+".txt";
	...
	private static String downLoadPic(String images){
		...
		for (String url : temp) {
			...
			String pic_path =  FileUtils.downLoadPic(path+SNetCnSpider.www_url, url,SNetCnSpider.www_url,false);
			buffer+=pic_path+",";
		}
		return buffer;
	}
	...
	private static class DetailProcessThread implements Runnable{
		...
		public void run() {
			try {
				Thread.sleep(RandomUtils.getRandomNum(2000));
