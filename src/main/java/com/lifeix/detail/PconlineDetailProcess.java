...
import static com.lifeix.spider.PconlineSpider.www_url;

public class PconlineDetailProcess {
private static final Logger LOGGER = LoggerFactory.getLogger(TopspeedcomDetailProcess.class);
	...
	public static void getDoveboxDataFromUrl(String url){
		...
		String pageUrl =url;
			...
			Elements images = document.select("#J-sPicList li img");
			for (int i = 0; i < images.size(); i++) {
				Element  element =  images.get(i);
				String src = element.attr("src");
				src = src.replace("_thumb", "_mthumb");
				if (!src.endsWith(".gif")) {
					image +=src+",";
				}
			}	
			
			Element tagsEle = document.getElementById("J-theme");
			if (tagsEle!=null) {
				tags = tagsEle.text();
			}
			
			Element descEle = document.getElementById("J-abstruct");
			if (descEle!=null) {
				desc = descEle.text();
			}
	...
	private static String downLoadPic(String images){
		...
		for (String url : temp) {
			...
			String pic_path =  FileUtils.downLoadPic(path+www_url, url,MeizitucomSpider.www_url,false);
			buffer+=pic_path+",";
		}
		return buffer;
	}
