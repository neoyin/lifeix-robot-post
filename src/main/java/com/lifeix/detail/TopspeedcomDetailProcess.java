...
private static final Logger LOGGER = LoggerFactory.getLogger(TopspeedcomDetailProcess.class);
	...
	public static void dowithData(){
		...
		String fileName = dir+"/temp_"+TopspeedcomSpider.www_url;
		...
		String pageUrl =url;
			...
			for (Element element : elements) {
				...
				if (name.endsWith("description")) {
					desc = content.replace(TopspeedcomSpider.www_url, "");
				}
			...
			Elements images = document.select(".wide_crop .wide");
	...
	private static void saveDoveData(String url,String str){
		...
		String fileName = path +TopspeedcomSpider.www_url+"/post_"+suffix+".txt";
	...
	private static String downLoadPic(String images){
		...
		for (String url : temp) {
			...
			String pic_path =  FileUtils.downLoadPic(path+TopspeedcomSpider.www_url, url,TopspeedcomSpider.www_url,false);
			buffer+=pic_path+",";
		}
		return buffer;
	}
