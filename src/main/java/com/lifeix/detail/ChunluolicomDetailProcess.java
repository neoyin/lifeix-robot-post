...
import com.lifeix.spider.ChunluolicomSpider;
...
public class ChunluolicomDetailProcess {
private static final Logger LOGGER = LoggerFactory.getLogger(ChunluolicomDetailProcess.class);
	...
	public static void dowithData(){
		...
		String fileName = dir+"/temp_"+ChunluolicomSpider.www_url;
		...
		String pageUrl =url;
			...
			for (Element element : elements) {
				...
				if (name.endsWith("description")) {
					desc = content.replace(ChunluolicomSpider.www_url, "");
				}
	...
	private static void saveDoveData(String url,String str){
		...
		String fileName = path +ChunluolicomSpider.www_url+"/post_"+suffix+".txt";
	...
	private static String downLoadPic(String images){
		...
		for (String url : temp) {
			...
			String pic_path =  FileUtils.downLoadPic(path+ChunluolicomSpider.www_url, url,ChunluolicomSpider.www_url,false);
			buffer+=pic_path+",";
		}
		return buffer;
	}
