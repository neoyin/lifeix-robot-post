...
import static com.lifeix.spider.PhotodomSpider.www_url;
...
public class PhotodomDetailProcess {
private static final Logger LOGGER = LoggerFactory.getLogger(PhotographyDetailProcess.class);
	...
	public static void getDoveboxDataFromUrl(String url){
		...
		try {
			Thread.sleep(RandomUtils.getRandomNum(9000));
		...
		String pageUrl =url;
			...
			for (Element element : elements) {
				...
				if (name.endsWith("keywords")) {
					tags = content;
					tags.replace("photodom,photo,image,","");
				}
				
			}
			Elements images = document.select("#main_image");
	...
	private static class DetailProcessThread implements Runnable{
		...
		public void run() {
			try {
				Thread.sleep(RandomUtils.getRandomNum(1000));
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
