...
import com.lifeix.spider.PhotographySpider;
...
public class PhotographyDetailProcess {
private static final Logger LOGGER = LoggerFactory.getLogger(PhotographyDetailProcess.class);
	...
	public static void dowithData(){
		...
		String fileName = dir+"/temp_"+PhotographySpider.www_url;
	...
	public static void getDoveboxDataFromUrl(String url){
		...
		try {
			Thread.sleep(RandomUtils.getRandomNum(3000));
		...
		String pageUrl =url;
			...
			for (Element element : elements) {
				...
				if (name.endsWith("description")) {
					desc = content.replace(PhotographySpider.www_url, "");
				}
			...
			Elements images = document.select(".primary_photo img");
			...
			title=document.select("#page_head h1").text();
	...
	private static void saveDoveData(String url,String str){
		...
		String fileName = path +PhotographySpider.www_url+"/post_"+suffix+".txt";
	...
	private static String downLoadPic(String images){
		...
		for (String url : temp) {
			...
			String pic_path =  FileUtils.downLoadPic(path+PhotographySpider.www_url, url,PhotographySpider.www_url,false);
			buffer+=pic_path+",";
		}
		return buffer;
	}
